...
gulp.task('watch', ['buildTest'], function (cb) {
  watch(cb);
});

gulp.task('default', ['buildTest'], function (cb) {
  watch(cb);
});
...
gulp.task('_compileSpecs', function (cb) {
  _compileSpecs(cb);
});

gulp.task('_deployMisc', function (cb) {
  _deployMisc(cb);
});
...
function _deployMisc(cb) {
  ...
  gulp.src(['package.json', 'LICENSE', 'README.md', 'src/systemjs.config.js', 'src/build.js', 'src/**/*.yml'])
...
function watch(cb) {
  ...
  _watchTypescript();
  gulp.watch(['package.json', 'LICENSE', 'README.md', 'src/test/*.js', '**/*.yml'], ['_deployMisc']);
