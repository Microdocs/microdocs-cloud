...
export class LoggerFactory {
  ...
  public static create( namespace?: string ): Logger {
    ...
    if ( LoggerFactory.config ) {
      LoggerFactory.applyConfig( logger, LoggerFactory.config );
    }
    return logger;
  }
  ...
  public static remove( namespace?: string ): void {
    LoggerFactory.loggerAdapter.removeLogger( namespace );
  }
  ...
  public static getLoggers( namespace?: string ): Logger[] {
    let loggers = LoggerFactory.loggerAdapter.list();
    if ( namespace ) {
  ...
  public static setLevel( logLevel: LogLevel, namespace?: string ): void {
    LoggerFactory.getLoggers( namespace ).forEach( logger => {
  ...
  public static configure( config: LoggerConfiguration ): void {
    ...
    LoggerFactory.getLoggers().forEach( logger => LoggerFactory.applyConfig( logger, config ) );
  }
